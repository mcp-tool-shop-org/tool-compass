...
**MCPツール用セマンティックナビゲーター - 記憶ではなく、意図に基づいて最適なツールを見つける**
...
MCPサーバーには、数十から数百のツールが公開されています。すべてのツールの定義をコンテキストに読み込むと、トークンを浪費し、応答速度が低下します。
...
初回実行時に検証済みのプラットフォームバイナリをダウンロードします（GitHubのリリースに対するSHA256チェック済み）。ローカルにキャッシュされるため、以降の実行は瞬時に開始されます。詳細は、npmの[@mcptoolshop/tool-compass](https://www.npmjs.com/package/@mcptoolshop/tool-compass)を参照してください。
...
> GHCRイメージ（`ghcr.io/mcp-tool-shop-org/tool-compass`）は、`linux/amd64`と`linux/arm64`をサポートしており、同じタグがx86_64サーバーとApple Silicon / ARMワークステーションで動作します。
...
Tool Compassは、トークンの使用量を最小限に抑えるために、3つの段階的な情報開示パターンを使用します。
...
- 段階的な情報開示では、使用するツールごとに約600トークン
- 削減効果：**典型的なワークフローでは95%以上**
...
| `TOOL_COMPASS_DATA_DIR` | データディレクトリ | プラットフォーム固有（詳細は下記参照） |
| `OLLAMA_URL` | OllamaサーバーURL | `http://localhost:11434` |
| `COMFYUI_URL` | ComfyUIサーバー | `http://localhost:8188` |
| `PORT` | HTTPトランスポートを有効にする（例：Fly.ioの場合） | 未設定（標準入出力） |
...
| クエリのレイテンシ | 約15ms（埋め込み処理を含む） |
| トークン削減量 | 約95% (38K → 2K) |
| 精度@3 | 約95%（上位3つのうち、正しいツールが含まれる） |
...
**原因:** `print()` 文がJSON-RPCプロトコルを破壊している。

**解決策:** ロギングを使用するか、`file=sys.stderr` を使用する。
```python
import sys
...
- **処理されるデータ:** ツール記述がローカルのHNSWベクトルデータベースにインデックス化され、検索クエリがローカルのSQLiteデータベース（`compass_analytics.db`）に記録され、埋め込みがローカルのOllamaで生成される。
- **処理されないデータ:** ユーザーコード、ファイルの内容、認証情報は含まれない。ツールの呼び出し引数はハッシュ化され、プレーンテキストで保存されない。
- **ネットワーク:** 埋め込み処理のためにローカルのOllamaに接続する。オプションのGradio UIはlocalhostにバインドされる。外部のテレメトリは送信されない。
- **テレメトリなし:** 外部に何も収集しない。分析はローカルでのみ行われる。

## スコアカード

カテゴリごとのスコアは、スウォーム完了後に以下のように再生成されます。
`bash scripts/regenerate-scorecard.sh` (これは `npx
@mcptoolshop/shipcheck audit` をラップする)。
現在の公式な内訳については、[SCORECARD.md](SCORECARD.md) を参照してください。
以下の表はそれを反映していますが、意図的に手動で作成されていません。
手動でキュレーションされたセクション（既知の欠点、修正履歴）は、SCORECARD.md内の`<!-- SHIPCHECK-AUTO-START/END -->`マーカーの外にあり、再生成後も保持されます。

| カテゴリ | スコア | 備考 |
|----------|-------|-------|
| A. セキュリティ | 未定 (TBD) | SHAで固定されたアクション; ダイジェストで固定されたベースイメージ; PyPI + GHCRでのSLSAプロビナンス + SBOM; pre-commitによるシークレットスキャン |
| B. エラー処理 | 未定 (TBD) | 構造化された結果、graceful degradation、終了コード |
| C. 運用ドキュメント | 未定 (TBD) | README、CHANGELOG、LICENSE、Makefile `verify` + `verify-metrics` + `scorecard` |
| D. ソフトウェアの品質 | 未定 (TBD) | CIの統合; 各ジョブでのタイムアウト時間 + 保存期間; pyproject.tomlでのpytestの設定 |
| E. 識別 (ソフト) | 未定 (TBD) | ロゴ、ランディングページ、GitHubのメタデータ; pyproject.tomlでの明示的なメンテナ |
| **Total** | **TBD** | `make scorecard` で再生成 |
