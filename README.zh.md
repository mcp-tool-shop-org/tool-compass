...
Tool Compass 使用**语义搜索**从自然语言描述中查找相关工具。与其加载所有工具，不如让 Claude 调用 `compass()`，并提供一个意图，然后返回相关的工具。
...
首次运行时，会下载经过验证的平台二进制文件（与 GitHub 发布中的 SHA256 校验值进行比较）。本地缓存——后续调用会立即启动。请参阅 npm 上的 [@mcptoolshop/tool-compass](https://www.npmjs.com/package/@mcptoolshop/tool-compass)。
...
> `linux/amd64` 和 `linux/arm64`，因此相同的标签可以在 x86_64 服务器
> 和 Apple Silicon / ARM 工作站上运行。

## 特性

- **混合搜索**——语义（HNSW）+ 词法融合，并具有精确名称增强——描述您想要的内容，或者粘贴工具名称，它将排名第一。
- **完整模式的逐步披露**——`compass()` → `describe()` → `execute()`；`describe()` 返回完整的 `inputSchema`（必需字段、描述、枚举、默认值）。
- **stdio + HTTP 后端**——支持本地子进程 MCP 服务器*以及*通过可流式传输的 HTTP 访问远程/SaaS 服务器，并可选地使用 bearer-token 身份验证。
- **每个工具的超时和允许/拒绝**——覆盖每个后端/工具的默认超时时间；公开广泛后端中的安全子集。
- **热缓存和链检测**——预加载常用工具；自动发现常见的工具工作流程。
- **分析**——跟踪使用模式和工具性能（具有保留/清理功能）。
- **跨平台和 Docker 兼容**——Windows、macOS、Linux；一键部署。
...
| 工具 | 描述 |
|------|-------------|
| `compass(intent)` | 具有精确名称增强的混合语义 + 词法搜索 |
| `describe(tool_name)` | 获取工具的完整 `inputSchema`（必需字段/枚举/默认值） |
| `execute(tool_name, args)` | 在其后端上运行工具 |
| `compass_categories()` | 列出类别和服务器 |
| `compass_status(active)` | 系统健康状况和配置；`active=True` 运行实时后端活动探测。 |
| `compass_analytics(timeframe)` | 使用情况统计 |
| `compass_chains(action)` | 管理工具工作流程 |
| `compass_sync(force)` | 从后端重新构建索引 |
| `compass_audit()` | 完整的系统报告 |

相同的操作也可以从 CLI 获得——包括 `tool-compass execute <tool> '<json>'`，用于从终端对代理调用进行测试。

### 逐步披露模式

Tool Compass 使用三步逐步披露模式来最大限度地减少令牌使用：
...
| `TOOL_COMPASS_DATA_DIR` | 数据目录 | 特定于平台（参见下文） |
...
| `TOOL_COMPASS_GATEWAY_AUTH_TOKEN` | HTTP 传输上所需的 bearer-token（可选；覆盖 `gateway_auth_token` 配置字段） | 未设置（无身份验证） |
...
v2.5.0 中添加的配置文件设置（在 `compass_config.json` 中）——`hybrid_search`、
`exact_name_boost`、每个后端的 `default_timeout` / `tool_timeouts`、
`allow_tools` / `deny_tools`、`analytics_retention_days` 以及 HTTP（`type: "http"`）
后端——在 [手册 → 配置](https://mcp-tool-shop-org.github.io/tool-compass/handbook/configuration/) 中进行了记录。
请参阅 [`.env.example`](.env.example) 以获取环境变量选项。
...
- [Backpropagate](https://github.com/mcp-tool-shop-org/backpropagate) - 无界面的 LLM 微调
...
Tool Compass 是一款**本地优先**的开发工具。请参阅 [SECURITY.md](SECURITY.md) 以获取完整策略。

- **涉及的数据：** 索引在本地 HNSW 向量数据库中的工具描述，搜索查询记录在本地 SQLite 数据库中（`compass_analytics.db`），通过本地 Ollama 生成的嵌入。
- **不涉及的数据：** 不涉及用户代码、文件内容或凭据。工具调用参数会被哈希处理，而不是以明文形式存储。
- **网络：** 连接到本地 Ollama 以进行嵌入。可选的 Gradio UI 绑定到 localhost。没有外部遥测。
- **无遥测：** 不收集任何外部数据。分析仅限于本地。

## 评分卡

每个类别的评分在完成集群后会重新生成，通过
`bash scripts/regenerate-scorecard.sh`（它封装了 `npx @mcptoolshop/shipcheck audit`）。请参阅 [SCORECARD.md](SCORECARD.md) 以获取
当前的权威细分——下表是对其的镜像，并且有意不是手动编写的。手动整理的部分（已知差距、修复历史记录）位于 SCORECARD.md 中 `<!-- SHIPCHECK-AUTO-START/END -->`
标记之外，并且在重新生成时会保留。

最新的 `shipcheck audit`：**32 个通过 · 0 个未通过 · 5 个跳过 · 100% 通过——所有硬性要求均已满足。**

| 类别 | 得分 | 备注 |
|----------|-------|-------|
| A. 安全 | ✅ 通过 | SHA 固定操作；摘要固定的基础镜像；SLSA 溯源 + PyPI + GHCR 上的 SBOM；预提交的密钥扫描；可选的网关承载者身份验证 |
| B. 错误处理 | ✅ 通过 | 结构化结果、优雅降级、退出代码 |
| C. 操作文档 | ✅ 通过 | README、CHANGELOG、LICENSE、Makefile `verify` + `verify-metrics` + `scorecard` |
| D. 发布规范 | ✅ 通过 | CI 整合；每个作业的超时时间 + 保留天数；pytest 配置在 pyproject.toml 中 |
| E. 身份（软性要求） | ✅ 通过 | 徽标、登录页面、GitHub 元数据；在 pyproject.toml 中明确的维护者 |
| **Total** | **100%** | 所有硬性要求均已满足——通过 `make scorecard` 重新生成 |
