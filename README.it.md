...
[Installazione](#quick-start) • [Utilizzo](#usage) • [Docker](#option-2-docker) • [Manuale](#handbook) • [Prestazioni](#performance) • [Contributi](#contributing)
...
- **Rivelazione progressiva:** `compass()` → `describe()` → `execute()`
- **Cache dinamica:** Gli strumenti utilizzati frequentemente vengono caricati in anticipo.
- **Rilevamento delle catene:** Scopre automaticamente i flussi di lavoro degli strumenti più comuni.
...
| `describe(tool_name)` | Ottiene lo schema completo di uno strumento. |
| `execute(tool_name, args)` | Esegue uno strumento sul suo backend. |
...
| `compass_chains(action)` | Gestisce i flussi di lavoro degli strumenti. |
| `compass_sync(force)` | Ricostruisce l'indice dai backend. |
| `compass_audit()` | Rapporto completo del sistema. |

### Modello di rivelazione progressiva

Tool Compass utilizza un modello di rivelazione progressiva in tre fasi per ridurre al minimo l'utilizzo dei token:
...
- Rivelazione progressiva = ~600 token per strumento utilizzato
- Risparmio: **95%+ per i flussi di lavoro tipici**
...
| `PORT` | Impostare per abilitare il trasporto HTTP (ad esempio, per Fly.io) | non impostato (stdio) |
...
| Latenza della query | ~15ms (inclusi gli embedding) |
| Risparmio di token | ~95% (38K → 2K) |
| Precisione al 3° posto | ~95% (strumento corretto tra i primi 3) |
...
Se i log di Claude Desktop mostrano errori di parsing JSON:
```
...
- **Dati elaborati**: descrizioni degli strumenti indicizzate in un database vettoriale HNSW locale, query di ricerca registrate in un database SQLite locale (`compass_analytics.db`), embedding generati tramite Ollama locale.
- **Dati NON elaborati**: nessun codice utente, nessun contenuto di file, nessuna credenziale. Gli argomenti delle chiamate agli strumenti sono hashati e non memorizzati in testo semplice.
- **Rete**: si connette a Ollama locale per gli embedding. L'interfaccia utente Gradio opzionale è legata a localhost. Nessuna telemetria esterna.
...
|----------|-------|-------|
| A. Sicurezza | 10/10 | SECURITY.md, solo locale, nessuna telemetria, SQL parametrizzato |
| B. Gestione degli errori | 10/10 | Risultati strutturati, fallback elegante di Ollama |
| C. Documentazione per gli operatori | 10/10 | README, CHANGELOG, CONTRIBUTING, documentazione API |
| D. Qualità del codice | 10/10 | CI (lint + test + coverage + pip-audit + Docker), script di verifica |
| E. Identità | 10/10 | Logo, traduzioni, pagina di presentazione |
| **Total** | **50/50** | |
